...
#show: jdf.with(
  title: "Joyner Document Format v2.2:
  For Use in CS6460, CS6750, and CS7637",
  author: "Author Name",
  email: "email@gatech.edu",
  paper-size: "us-letter", // Or change to "a4"
  abstract: [
...
#link("https://www.ctan.org/tex-archive/fonts/tex-gyre/opentype")[TeX~Gyre Pagella]
...
#strong[Bold] and #emph[italics] should be used for emphasis. Hyperlinks
may be inserted in the text, as well as `in-line code`, superscripts
#super[like this], and subscripts #sub[like this].
...
Word, Pages, and LaTeX~will do this automatically when you use the
appropriate paragraph styles, but Google Docs users will need to number
their headings manually. Headings should not span more than one line.

=== Heading 1
<heading-1>
Heading 1 should be set in all caps, with letterspacing expanded by 0.66
points (6%). It should have 11 points of space added before and 8.5
points of space added after.
...
5.5\" x 8.5\", which is exactly $1 / 2$ the size of the page, divided
lengthwise.
...
graph), it may aid legibility to add a $1 / 4$-point black border

Figure captions should be centered beneath the corresponding figure. The
label (e.g. "Figure 1") should be set in bold italics followed by an em
dash, and the entire caption should be 8.5 pt with 14 pt of line
spacing. The #emph[Figure Caption] style in Word will number your
figures automatically. If need be, you may have one figure caption
corresponding to consecutive figures and use either locational
descriptors (e.g. "top left," "middle") or labels (e.g. "A", "B") to map
parts of the caption to parts of the figure. Make sure captions fall on
the same page as the corresponding figure or table; you may rearrange
text to make this work.

#figure(image("Figures/flowchart.png", height: 6cm),
  caption: [
    Make sure your flowcharts are more useful than this one. Source:
    #link("https://xkcd.com/1195/")[XKCD].
  ]
)
<fig:flowchart>

In Word, you may need to either change the image's text wrap settings to
"Top and Bottom" or change the line spacing of the image to 1.0.
...
#figure(
  ...
  table(
    columns: (0.17fr, 0.12fr, 0.17fr, 0.4fr),
    align: (left, center, center, left),
    stroke: none, // Remove default borders
    ...
    // Rows
    [Golden ratio], [$phi$], [1.618], [Number such that the ratio of the number is equal to the ratio of its reciprocal to 1],
...
types not covered here, you have flexibility in determining how it
...
some expert grading where possible."
]
...
are preferred over footnotes, and we favor APA citation format for both
in-line citations and reference lists. Refer to the Purdue Online
...
research finds sentiment analysis of the text of OMSCS reviews
corresponds to student-assigned ratings of the course @newman2018. You
may also cite multiple studies together. For example: several studies
have found students in the online version of an undergraduate CS1 class
performed equally with students in a traditional version (@joyner2018a\;
@joyner2018b). If you would like to refer to an author in text, you may
also do so by including the year (in parentheses) after the author's
name in the text. If a publication has more than 4 authors, you may list
the first author followed by 'et al.' For example:
#cite(<joyner2016>, form: "prose") claim that a round of peer review
prior to grading may improve graders' efficiency and the
quality of feedback given. This applies to parenthetical citations
as well, e.g. @joyner2016.
...
#bibliography("references.bib", style: "jdf-numeric-alpha.csl", title: none)
